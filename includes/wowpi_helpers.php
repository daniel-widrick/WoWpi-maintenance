...
function wowpi_buildServiceUrl($dataType, $dataRequested) {
    $serviceUrl = 'https://'.wowpi_getRegion().'.api.battle.net/wow/data/'.$dataType.'/'.$dataRequested.'?locale='.wowpi_getLocale().'&apikey='.wowpi_getApiKey();
    return $serviceUrl;
}
...
function wowpi_getRaces() {
    ...
    if(($returned==false) || (isset($returned['last_update']) && (intval($returned['last_update'])+(62*24*60*60))<current_time('timestamp'))) {
        ...
        $serviceUrl = 'https://' . wowpi_getRegion() . '.api.battle.net/wow/data/character/races?locale=' . wowpi_getLocale() . '&apikey=' . wowpi_getApiKey();
...
function wowpi_getClasses() {
    ...
    if(($returned==false) || (isset($returned['last_update']) && (intval($returned['last_update'])+(62*24*60*60))<current_time('timestamp'))) {
        $serviceUrl = 'https://' . wowpi_getRegion() . '.api.battle.net/wow/data/character/classes?locale=' . wowpi_getLocale() . '&apikey=' . wowpi_getApiKey();
        $curl_response = wowpi_retrieve_data($serviceUrl);
        $decoded = json_decode($curl_response);
...
function wowpi_getArtifact($artifactId)
{
    ...
    if(($returned==false) || (isset($returned['last_update']) && (intval($returned['last_update'])+(62*24*60*60))<current_time('timestamp'))) {
        $serviceUrl = 'https://' . wowpi_getRegion() . '.api.battle.net/wow/item/'.$artifactId.'?locale=' . wowpi_getLocale() . '&apikey=' . wowpi_getApiKey();
        $curl_response = wowpi_retrieve_data($serviceUrl);
        $decoded = json_decode($curl_response);
        if(!isset($decoded)) {
            return $returned['data'];
...
function wowpi_getSpell($spellId)
{
    ...
    if(($returned==false) || (isset($returned['last_update']) && (intval($returned['last_update'])+(62*24*60*60))<current_time('timestamp'))) {
        $serviceUrl = 'https://' . wowpi_getRegion() . '.api.battle.net/wow/spell/'.$spellId.'?locale=' . wowpi_getLocale() . '&apikey=' . wowpi_getApiKey();
        $curl_response = wowpi_retrieve_data($serviceUrl);
        $decoded = json_decode($curl_response);
        if(!isset($decoded)) {
            return $returned['data'];
...
function wowpi_getRealms($region = null, $locale = null) {
    ...
    if(($returned==false) || (isset($returned['last_update']) && (intval($returned['last_update'])+(62*24*60*60))<current_time('timestamp'))) {
        ...
        $serviceUrl = 'https://' . $region . '.api.battle.net/wow/realm/status?locale=' . $locale . '&apikey=' . wowpi_getApiKey();
        $curl_response = wowpi_retrieve_data($serviceUrl);
        $decoded = json_decode($curl_response);
...
function wowpi_getCharacterAchievements() {
    ...
    if(($returned==false) || (isset($returned['last_update']) && (intval($returned['last_update'])+(62*24*60*60))<current_time('timestamp'))) {
        $serviceUrl = 'https://' . wowpi_getRegion() . '.api.battle.net/wow/data/character/achievements?locale=' . wowpi_getLocale() . '&apikey=' . wowpi_getApiKey();
        $curl_response = wowpi_retrieve_data($serviceUrl);
        $decoded = json_decode($curl_response);
...
function wowpi_getGuildAchievements() {
    ...
    if(($returned==false) || (isset($returned['last_update']) && (intval($returned['last_update'])+(62*24*60*60))<current_time('timestamp'))) {
        $serviceUrl = 'https://' . wowpi_getRegion() . '.api.battle.net/wow/data/guild/achievements?locale=' . wowpi_getLocale() . '&apikey=' . wowpi_getApiKey();
        $curl_response = wowpi_retrieve_data($serviceUrl);
        $decoded = json_decode($curl_response);
        if(!isset($decoded->achievements)) {
            return $returned['data'];
...
function wowpi_getCharacterData($characterName = null, $realmName = null, $region = null, $locale = null) {
    ...
    if(($returned==false) || (isset($returned['last_update']) && (intval($returned['last_update'])+$characterCaching)<current_time('timestamp'))) {
        ...
        $serviceUrl = 'https://' . $region . '.api.battle.net/wow/character/'.$realmSlug.'/'.$characterName.'?fields='.$fieldsStr.'&locale=' . $locale . '&apikey=' . wowpi_getApiKey();
        $curl_response = wowpi_retrieve_data($serviceUrl);
        $decoded = json_decode($curl_response);
...
function wowpi_getGuildData($guildName = null, $realmName = null, $region = null, $locale = null)
{
    ...
    if(($returned==false) || (isset($returned['last_update']) && (intval($returned['last_update']) + $guildCaching) < current_time('timestamp'))) {
        ...
        $serviceUrl = 'https://' . $region . '.api.battle.net/wow/guild/' . $realmSlug . '/' . $guildName . '?fields=' . $fieldsStr . '&locale=' . $locale . '&apikey=' . wowpi_getApiKey();
        $curl_response = wowpi_retrieve_data($serviceUrl);
        $decoded = json_decode($curl_response);
        /*
        echo '<pre>';
